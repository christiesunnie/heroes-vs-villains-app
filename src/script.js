...
app.$form = $("form");
app.$listContainer = $(".list-container");
app.$btnFight = $(".btn-fight");
app.$message = $(".message");
...
app.getDataResponse = function (name) {
    $.ajax({
        ...
        .then((res) => {
            ...
            const result = app.randomIndex(resultsArray);

            // Call the data function to check if the data is the result of hero or villain
            app.getDataFromArray(result);

            // Call the function to search if there are enough of 10 <span> elements to get all of the values
            app.displayDataFromSpans();
        })
        ...
        .fail(() => {
            ...
            // Pop up the error message to UI for wrong search input
            app.$errorMessage.text(
                "Your Heroes or Villains not matched! Please enter the name in the right category!"
            );
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Create the random function to get the random result from the array of data
app.randomIndex = function (array) {
    const index = Math.floor(Math.random() * array.length);
    return array[index];
};

// Create the data function to check if the data is the result of hero or villain
app.getDataFromArray = function (data) {
    const type = data.biography.alignment;
    if (type === "good") {
        // Display the result to hero section
        app.displayInfo("hero", data);
...
app.displayDataFromSpans = function () {
    if ($(".fight").length === 10) {
        ...
        app.$btnFight.on("click", function () {
            app.$message.addClass("show");
...
app.displayMessage = function (type1, type2, score) {
  ...
  `;
    app.$message.append(messageHtml);
};
...
app.init = function () {
    ...
    $btnStart.on("click", () => scrollSToSec($btnStart, app.$form));

    // Listen to the submit form to scroll to the list section
    app.$form.on("submit", () => scrollSToSec(app.$listContainer, app.$form));

    // Listen to the fight button click to scroll to the score section
    app.$btnFight.on("click", () => scrollSToSec(app.$btnFight, app.$message));
};
